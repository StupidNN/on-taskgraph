...
function swaggerFactory(
    ...
    function swaggerController(options, callback) {
        ...
        return function (req, resÒ, next) {
            ...
            return Promise.try(function () {
                ...
                return callback(req, resÒ);
            }).then(function (result) {
                if (!resÒ.headersSent && result) {
                    if (_.isArray(result)) {
                        resÒ.body = result.map(function (element) {
                            if (toSort) {
                        ...
                        if (toSort) {
                            (toSort.type === 'string') ? resÒ.body.sort(_sortByAny(toSort.sortBy, toSort.dsc, function (a) { return a.toUpperCase() })) : resÒ.body.sort(_sortByAny(toSort.sortBy, toSort.dsc, parseInt)); // jshint ignore:line

                        }
                    } else {
                        resÒ.body = result.toJSON ? result.toJSON() : result;
                    }
                }
                if (!resÒ.headersSent) {
                    var operation = req.swagger.operation;
                    swaggerRenderer(req, resÒ, operation['x-view'], next);
                }
            }).catch(function (err) {
    ...
    function swaggerSerializer(injectableSerializer) {
        ...
        return function (req, resÒ, next) {
            ...
            if (_.isArray(resÒ.body)) {
                serialized = Promise.map(resÒ.body, function (item) {
                    return serialize(item);
                });
            } else {
                serialized = serialize(resÒ.body);
            }
    ...
    function _render(viewName, req, resÒ) {
        var options;
        return Promise.try(function () {
            ...
            .then(function () {
                if (_.isEmpty(resÒ.body) || !viewName) {
                    return resÒ.body;
                } else if (_.isArray(resÒ.body)) {
                    // Use ejs render directly to avoid repeatedly loading the same view
                    return Promise.try(function () {
                        return views.get(viewName);
                    })
                        .then(function (view) {
                            return Promise.map(resÒ.body, function (element) {
                    ...
                    return views.render(viewName, _.merge(resÒ.body, options));
                }
            })
            .then(function (data) {
                ...
                if (typeof (data) === 'object') {
                    resÒ.set('Content-Type', 'application/json');
                    ...
                    body = Promise.try(function () {
                        ...
                        .catch(function (err) {
                            if (!viewName) {
                                resÒ.set('Content-Type', 'text/plain');
                                return data;
                            } else {
    ...
    function swaggerRenderer(req, resÒ, viewName, next) {
        return Promise.try(function () {
            assert.ok(!resÒ.headersSent, 'headers have already been sent');
            return [viewName, req, resÒ];
        })
            .spread(_render)
            .tap(function () {
                // Set appropriate HTTP status.
                if (_.isEmpty(resÒ.body) && req.swagger.options.send204OnEmpty) {
                    resÒ.status(204);
                } else {
                    resÒ.status(resÒ.locals.errorStatus || req.swagger.options.success || 200);
                }
            })
            .then(resÒ.send.bind(resÒ))
            .catch(function (err) {
                if (resÒ.locals.errorStatus) {
                    next();
                } else {
                    next(_processError(err));
                }
    ...
    function makeRenderableOptions(req, resÒ, context, ignoreLookup) {
        var scope = resÒ.locals.scope;
        ...
        return Promise.try(function () {
            //实时查询arp缓存替换掉查询mongodb的不准确的缓存
            // return ignoreLookup ? '' : lookupService.ipAddressToMacAddress(resÒ.locals.ipAddress);
            if (req.query.nodeId || req.query.macs) {
                let id = req.query.nodeId || req.query.macs;
                return nodeApiService.getNodeByIdentifier(id);
            } else {
                return find(req.ip);
            }
        })
    ...
    function _addLinksHeader(req, resÒ, count) {
    ...
    function addLinksHeader(req, resÒ, collection, query) {
        return waterline[collection].count(query)
            .then(function (count) {
                return _addLinksHeader(req, resÒ, count);
            });
    }
